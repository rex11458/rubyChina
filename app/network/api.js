...
var BASE_URL = 'https://ruby-china.org/api/v3/';
...
function api(api, v){
	...
	return BASE_URL + api + '?access_token=' + ACCESS_TOKEN + '&' + p;
}
...
function getNodeTopics(offset, limit,node_id)
{
	if (node_id && node_id !== '') {
		return api('topics.json', {'node_id':node_id, 'offset':offset, 'limit':limit});
	}
	return getHomeTopics(offset,limit);
}
